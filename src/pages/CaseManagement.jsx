...
const CaseManagement = () => {
  ...
  // Check for pre-selected case ID from navigation (e.g., from Dashboard)
  const initialCaseId = location.state?.caseId;
  const initialCase = initialCaseId ? cases.find(c => c.id === initialCaseId) : null;
  ...
  const handleFileChange = (event) => {
    ...
    const newDoc = {
      ...
      url: URL.createObjectURL(file) // Create ephemeral URL
    };

    const updated = cases.map(c =>
      c.id === selectedCase.id ? { ...c, documents: [...c.documents, newDoc] } : c
    );
  ...
  const handleViewDocument = (doc) => {
    if (doc.url) {
      ...
      // Fallback for mock data that doesn't have real URLs
      alert(`Viewing ${doc.name} (Simulation)`);
    }
  };
          ...
          <div className="overflow-y-auto flex-1 p-2 space-y-2">
            {filteredCases.map(c => (
              <div
                ...
                className={`p-4 rounded-lg cursor-pointer border transition-all ${selectedCase?.id === c.id
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full border ${c.status === 'Active'
